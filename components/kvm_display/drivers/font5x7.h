 ...
 *
 * A 5x7 monospace font for printable ASCII 0x20-0x7F, column-major: five bytes
 * per glyph, one per column left-to-right, bit 0 = top pixel. The classic public
 * "font5x7" table. Shared by every driver that draws text - the mono OLED blits
 * it 1:1, a colour panel scales it - so the glyphs live in one place. Header-only
 * and static, so each translation unit that needs it gets its own small copy.
 */
#pragma once
